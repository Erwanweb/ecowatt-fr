...
class BasePlugin:
    ...
    def onStart(self):
        ...
        devicecreated = []
        if 1 not in Devices:
            Domoticz.Device(Name="Actuellement", Unit=1, TypeName="Alert", Used=1).Create()
            devicecreated.append(deviceparam(1, 0, ""))  # default is clear
        if 2 not in Devices:
            Domoticz.Device(Name="J-0", Unit=2, TypeName="Alert", Used=1).Create()
            devicecreated.append(deviceparam(2, 0, ""))  # default is clear
        if 3 not in Devices:
            Domoticz.Device(Name="J+1", Unit=3, TypeName="Alert", Used=1).Create()
            devicecreated.append(deviceparam(3, 0, ""))  # default is clear
        if 4 not in Devices:
            Domoticz.Device(Name="J+2", Unit=4, TypeName="Alert", Used=1).Create()
            devicecreated.append(deviceparam(4, 0, ""))  # default is clear
        if 5 not in Devices:
            Domoticz.Device(Name="J+3", Unit=5, TypeName="Alert", Used=1).Create()
            devicecreated.append(deviceparam(5, 0, ""))  # default is clear
    ...
    def onHeartbeat(self):
        ...
        if self.EcoWattRequest <= now:
            ...
            if RTEDatas :
                ...
                # Check Prod type now if no alert
                #self.NOW = datetime.now()
                ...
                if self.ProdNow == 0 :
                    self.ProdType = "Pas d'alerte + Production décarbonée."
                else :
                    self.ProdType = self.J0Message
                Domoticz.Debug("Prod type now is level {} and in text {}".format(str(self.ProdNow), str(self.ProdType)))

                #Check day name for j+2 and j+3
                ...
                if (Today-5) == 1 :
                    ...
                    J2 = "Samedi"
                    J3 = "Dimanche"
                Domoticz.Debug("J+2: {} - J+3: {}".format(J2, J3))

                #Updating devices values
                Domoticz.Log("Updating Devices from RTE datas")
                Devices[1].Update(nValue=self.J0Value, sValue="Actuellement : {}".format(str(self.J0Message)))
                Devices[2].Update(nValue=self.J0Value, sValue="Aujourdhui : {}".format(str(self.J0Message)))
                Devices[3].Update(nValue=self.J1Value, sValue="Demain : {}".format(str(self.J1Message)))
                Devices[4].Update(nValue=self.J2Value, sValue="{} : {}".format(J2, str(self.J1Message)))
                Devices[5].Update(nValue=self.J3Value, sValue="{} : {}".format(J3, str(self.J1Message)))

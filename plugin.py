    ...
    def onHeartbeat(self):
        ...
        if self.EcoWattRequest <= now:
            ...
            if RTEDatas :
                ...
                Domoticz.Debug("in day 0 now we are the hour number {} ".format(str(self.HourNumber)))
                self.ProdNow = int(RTEDatas['signals'][0]['values'][self.HourNumber]['hvalue'])
                if self.ProdNow == 0 :
                    self.ProdType = " + Production décarboné"
                else :
                    self.ProdType = ""
                ...
                Devices[1].Update(nValue=self.J0Value, sValue="{}{}".format(str(self.J0Message)), (str(self.ProdType)))

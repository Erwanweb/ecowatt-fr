    ...
    def onHeartbeat(self):
        ...
        if self.EcoWattRequest <= now:
            ...
            if RTEDatas :
                ...
                self.J0Value = int(RTEDatas[0]['dvalue'])
                self.J0Message = str(RTEDatas[0]['message'])
...
def EcoWattAPI(APICall):
    ...
    #with open(jsonFile, decoding='UTF-8') as EcoWattStream:
    with open(jsonFile) as EcoWattStream:
        try:
            #RTEjsonData = json.load(EcoWattStream)
            RTEjsonData = json.load(EcoWattStream.decode('utf-8'))
            #RTEjsonData = json.loads(EcoWattStream.read().decode('utf-8'))
        except:
    ...
    resultJson = None
    url = f"/home/tools/onevar/ecowatt.json{parse.quote(APICall, safe='&=')}"
    ...
    try:
        ...
        if response.status == 200:
            resultJson = json.loads(response.read().decode('utf-8'))
            if resultJson.get("status") != "OK":
                Domoticz.Error(f"EcoWatt API returned an error: status = {resultJson.get('status')}")
                resultJson = None
        else:
            Domoticz.Error(f"EcoWatt API: HTTP error = {response.status}")
